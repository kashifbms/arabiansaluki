/* =====================================================================
   View: Breeder Registry — kennels + litter registration (Module 10)
   ===================================================================== */
...
ASC.views.breeder = function (root) {
  ...
  function statHTML() {
    return sc('lineage', 'Licensed Breeders', B.length) + sc('paw', 'Litters Registered', L.filter((l) => l.status === 'Registered').length) +
      sc('id', 'Puppies Registered', P.length) + sc('clock', 'Expected Litters', L.filter((l) => l.status !== 'Registered').length);
  }
  function kennelRows() {
    return B.map((b) =>
      ...
      '<td>' + ASC.ui.ownerChip(b.owner) + '</td><td class="muted">' + b.emirate + '</td><td class="mono">' + b.license + '</td>' +
      '<td class="mono">' + b.expiry + '</td><td class="mono">' + b.litters + '</td><td>' + tag(b.status, b.status === 'Active') + '</td></tr>').join('');
  }
  function litterRows() {
    return L.map((l) =>
  ...
  const pups = P.slice(0, 4).map((p) =>
    '<div class="ped-node"><div class="ped-role">' + p.id + '</div><div class="ped-name">' + p.name + '</div><div class="ped-reg">' + p.sex + ' · ' + p.color + '</div></div>').join('');

  ASC.refreshBreeder = function () {
    const s = document.getElementById('brStrip'); if (s) { s.innerHTML = statHTML(); ASC.hydrateIcons(s); }
    const k = document.getElementById('kennelBody'); if (k) { k.innerHTML = kennelRows(); ASC.hydrateIcons(k); }
    const lt = document.getElementById('litterBody'); if (lt) lt.innerHTML = litterRows();
  };
  ...
  root.innerHTML =
    ...
    '<div class="statstrip" id="brStrip">' + statHTML() + '</div>' +
    '<div class="card table-card" style="margin-bottom:var(--gutter)"><div class="card-head" style="padding:16px 16px 0"><div class="card-title">Licensed Kennels</div></div>' +
      '<div class="table-scroll"><table class="data-table"><thead><tr><th>Kennel</th><th>Owner</th><th>Emirate</th><th>License No</th><th>Expiry</th><th>Litters</th><th>Status</th></tr></thead><tbody id="kennelBody">' + kennelRows() + '</tbody></table></div></div>' +
    '<div class="card table-card" style="margin-bottom:var(--gutter)"><div class="card-head" style="padding:16px 16px 0"><div class="card-title">Litter Registry</div></div>' +
      '<div class="table-scroll"><table class="data-table"><thead><tr><th>Litter ID</th><th>Kennel</th><th>Pairing (Sire × Dam)</th><th>Born</th><th>Puppies</th><th>Genetic Screening</th><th>Status</th></tr></thead><tbody id="litterBody">' + litterRows() + '</tbody></table></div></div>' +
  ...
  root.querySelector('#bAdd').addEventListener('click', () => ASC.openLitterModal());
};
